...
test.describe('NG-Simplicity Forms Material Demo E2E Tests', () => {
  ...
  test('should render form structure, section titles, and initial components', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Dynamic registration profile');
    ...
    await expect(page.getByText('Yes, sign me up for the weekly digest newsletter')).toBeVisible();
  });
  ...
  test('should dynamically toggle visibility of contact preference fields', async ({ page }) => {
    ...
    await page.getByText('None / No contact').click();
  ...
  test('should dynamically toggle visibility of nested newsletter form group', async ({ page }) => {
    ...
    await expect(page.getByText('I agree to the promotional email terms and conditions')).not.toBeVisible();

    // Check "sign me up" checkbox
    await page.getByText('Yes, sign me up for the weekly digest newsletter').click();

    // Frequency and Terms should now be visible
    await expect(page.getByLabel('Delivery Frequency')).toBeVisible();
    await expect(page.getByText('I agree to the promotional email terms and conditions')).toBeVisible();
  ...
  test('should toggle submitted state on submit click', async ({ page }) => {
    await expect(page.getByText('Submitted').locator('..').locator('.status-pill')).toHaveText('NO');
    ...
    await expect(page.getByText('Submitted').locator('..').locator('.status-pill')).toHaveText('YES');
  });
  ...
  test('should display validation error messages and highlights on invalid submit or bad input', async ({ page }) => {
    ...
    // 4. Provide invalid input (too short name) and verify message switches
    ...
    // 5. Provide valid input, and verify the highlight and error messages disappear
